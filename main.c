// Pong in C (raylib) - complete code
#include "raylib.h"
...
int main(void)
{
    ...
    while (!WindowShouldClose())
    {
        ...
        if (!game_started && IsKeyPressed(KEY_ENTER)) game_started = true;
        if (game_started)
        {
            // Paddle input
            float paddleSpeed = 600.0f;
            if (IsKeyDown(KEY_W))    l_y -= paddleSpeed * dt;
            if (IsKeyDown(KEY_S))    l_y += paddleSpeed * dt;
            if (IsKeyDown(KEY_UP))   r_y -= paddleSpeed * dt;
            if (IsKeyDown(KEY_DOWN)) r_y += paddleSpeed * dt;
            // Clamp paddles
            if (l_y < 0) l_y = 0;
            if (l_y + paddleH > H) l_y = H - paddleH;
            if (r_y < 0) r_y = 0;
            if (r_y + paddleH > H) r_y = H - paddleH;
            // Move ball
            ball_x += ball_speed_x * dt;
            ball_y += ball_speed_y * dt;
            // Wall bounce (top/bottom)
            if (ball_y - ballR <= 0)   { ball_y = ballR;      ball_speed_y *= -1; }
            if (ball_y + ballR >= H)   { ball_y = H - ballR;  ball_speed_y *= -1; }
            ...
            {
                score_r++;
                ball_x = W/2.0f; ball_y = H/2.0f;
                ball_speed_x = 350.0f; ball_speed_y = 350.0f;
            }
            if (ball_x - ballR > W)
            {
                score_l++;
